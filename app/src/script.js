...
function addFFmpegCommandScripts(start, duration, outputName, psScriptLines, bashScriptLines, fileListLines) {
...
function outputConcatScriptsToUI(psScriptLines, bashScriptLines, fileListLines) {
  psScriptLines.push(
    '\n@"\n' +
      fileListLines.join("\n") +
...
async function cutVideo() {
  ...
  if (isElectron) {
    ...
    for (let i = 0; i < totalParts; i++) {
      ...
      addFFmpegCommandScripts(start, duration, outputName, psScriptLines, bashScriptLines, fileListLines);
    ...
    for (let batchStart = 0; batchStart < totalParts; batchStart += BATCH_SIZE) {
      ...
      await concatSegments(segmentFileNames, `final_batch_${batchStart / BATCH_SIZE}.mp4`);
      const batchData = await ffmpeg.readFile(`final_batch_${batchStart / BATCH_SIZE}.mp4`);
  ...
  title.innerText = "✅ Done! You can now concatenate the parts or download scripts.";
}
